...
class InteractiveGameBoard():
    ...
    width: int
    heieght: int
    graph: list[list[ChessPiece, PotentialCaptures, PotentialMoves]]
    ...
    def isclicked(self, mousePos: tuple[int,int]):
        ...
        curSquare = self.graph[curSquareLoc]
        curObject = self._getObjectCurSquare()
        
        if isinstance(curObject,ChessPiece):
        
        if isinstance(curObject, PotentialMoves):
            curObject.isClicked()
        if isinstance(curObject,PotentialCaptures):
            curObject.isClicked()
